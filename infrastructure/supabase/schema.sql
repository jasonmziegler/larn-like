...
CREATE INDEX idx_heroes_player_alive ON heroes(player_id, is_alive);
CREATE INDEX idx_heroes_location ON heroes(current_level, position_x, position_y);

-- Equipment items table indexes
CREATE INDEX idx_equipment_type ON equipment_items(type);
CREATE INDEX idx_equipment_blessed ON equipment_items(is_blessed);
CREATE INDEX idx_equipment_owner ON equipment_items(original_owner);

-- Dungeon levels table indexes
CREATE INDEX idx_levels_depth ON dungeon_levels(depth);
CREATE INDEX idx_levels_regeneration ON dungeon_levels(last_regenerated);

-- Monsters table indexes
CREATE INDEX idx_monsters_level ON monsters(current_level);
CREATE INDEX idx_monsters_position ON monsters(current_level, position_x, position_y);
CREATE INDEX idx_monsters_queue ON monsters(queue_position) WHERE queue_position IS NOT NULL;
CREATE INDEX idx_monsters_evolved ON monsters(is_evolved, evolution_level);

-- Soul shrines table indexes
CREATE INDEX idx_shrines_level ON soul_shrines(level_depth);
CREATE INDEX idx_shrines_active ON soul_shrines(is_active);
CREATE INDEX idx_shrines_queue ON soul_shrines(queue_position) WHERE queue_position IS NOT NULL;
CREATE UNIQUE INDEX idx_soul_shrines_unique_position ON soul_shrines(level_depth, position_x, position_y) WHERE is_active = TRUE;

-- Death events table indexes
CREATE INDEX idx_deaths_time ON death_events(processed_at);
CREATE INDEX idx_deaths_location ON death_events(death_level, death_x, death_y);
CREATE INDEX idx_deaths_monster ON death_events(killer_monster_id);

-- Additional concurrent indexes for performance optimizations

-- ============================================================
-- SECTION 4: VIEWS FOR COMMON QUERIES
-- ============================================================

-- View for active monsters (not in queue)
...
-- Note: The evolved_monsters_summary view references a monster_kills table
-- that is not defined in the schema. This view is commented out until
-- the monster_kills table is properly defined.
--
...
ALTER TABLE dungeon_levels ENABLE ROW LEVEL SECURITY;
ALTER TABLE monster_types ENABLE ROW LEVEL SECURITY;
ALTER TABLE monsters ENABLE ROW LEVEL SECURITY;
ALTER TABLE soul_shrines ENABLE ROW LEVEL SECURITY;
...
-- Hero equipment policies (follows hero ownership)
CREATE POLICY "Hero equipment follows hero ownership" ON hero_equipment
FOR ALL USING (
  hero_id IN (
    SELECT id FROM heroes WHERE player_id IN (
      SELECT id FROM players WHERE auth_id = auth.uid()::text
    )
  )
);
...
CREATE POLICY "Monster types readable" ON monster_types
FOR SELECT TO authenticated USING (true);

CREATE POLICY "Monster data readable" ON monsters
FOR SELECT TO authenticated USING (true);

CREATE POLICY "Soul shrines readable" ON soul_shrines
FOR SELECT TO authenticated USING (true);

CREATE POLICY "Death events readable" ON death_events
FOR SELECT TO authenticated USING (true);
